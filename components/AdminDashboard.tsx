...
<html lang="en">
  ...
  // --- VERCEL LINKS HANDLER ---
  const handleUpdateExternalLink = (index: number, field: 'name' | 'url', value: string) => {
      const links = localSettings.externalLinks ? [...localSettings.externalLinks] : [];
      // Ensure array has enough size
      while (links.length <= index) {
          links.push({ id: `link-${Date.now()}-${Math.random()}`, name: '', url: '' });
      }
      
      const current = links[index] || { id: `link-${Date.now()}`, name: '', url: '' };
      links[index] = { ...current, [field]: value };
      
      // Limit to 10
      const trimmed = links.slice(0, 10);
      setLocalSettings({ ...localSettings, externalLinks: trimmed });
  };
  ...
  return (
    <div className="pb-20 bg-slate-50 min-h-screen">
      ...
      {activeTab === 'DASHBOARD' && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 mb-6 animate-in fade-in">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                  ...
                  <DashboardCard icon={Link} label="Vercel Pages" onClick={() => setActiveTab('VERCEL_LINKS')} color="orange" />
      ...
      {/* --- VERCEL LINKS (EXTERNAL LINKS) --- */}
      {activeTab === 'VERCEL_LINKS' && (
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 animate-in slide-in-from-right">
              <div className="flex items-center gap-4 mb-6">
                  <button onClick={() => setActiveTab('DASHBOARD')} className="bg-slate-100 p-2 rounded-full hover:bg-slate-200"><ArrowLeft size={20} /></button>
                  <h3 className="text-xl font-black text-slate-800">Manage Vercel Pages (Max 10)</h3>
              </div>
              <div className="space-y-4">
                  {Array.from({ length: 10 }).map((_, index) => {
                      const link = localSettings.externalLinks?.[index] || { id: '', name: '', url: '' };
                      return (
                          <div key={index} className="flex gap-4 items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
                              <span className="font-bold text-slate-400 w-6">#{index + 1}</span>
                              <div className="flex-1 space-y-2">
                                  <input type="text" placeholder="Page Name (e.g. Exam Results)" value={link.name} onChange={e => handleUpdateExternalLink(index, 'name', e.target.value)} className="w-full p-2 border rounded-lg text-sm font-bold" />
                                  <input type="text" placeholder="Vercel URL (https://...)" value={link.url} onChange={e => handleUpdateExternalLink(index, 'url', e.target.value)} className="w-full p-2 border rounded-lg text-sm text-blue-600" />
                              </div>
                          </div>
                      );
                  })}
                  <button onClick={handleSaveSettings} className="w-full bg-green-600 text-white font-bold py-4 rounded-xl shadow-lg hover:bg-green-700 flex items-center justify-center gap-2 mt-4">
